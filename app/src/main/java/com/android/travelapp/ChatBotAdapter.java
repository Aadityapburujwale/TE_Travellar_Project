...
public class ChatBotAdapter extends RecyclerView.Adapter{
    ...
    ChatBotAdapter(ArrayList<ChatBotModel> messageModalArrayList, Context context){
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case 0:
                ...
                view = LayoutInflater.from(parent.getContext()).inflate(R.layout.user_chat_adapter_layout, parent, false);
                return new ChatBotAdapter.UserViewHolder(view);
            case 1:
                // below line we are inflating bot message layout.
                view = LayoutInflater.from(parent.getContext()).inflate(R.layout.bot_chat_adapter_layout, parent, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        ChatBotModel modal = messageModalArrayList.get(position);
        switch (modal.getMessageSender()) {
            case "user":
                // below line is to set the text to our text view of user layout
                ((ChatBotAdapter.UserViewHolder) holder).userTV.setText(modal.getMessage());
                break;
            case "bot":
                // below line is to set the text to our text view of bot layout
                ((ChatBotAdapter.BotViewHolder) holder).botTV.setText(modal.getMessage());
                break;
        }
    }
    ...
    public int getItemViewType(int position) {
        ...
        switch (messageModalArrayList.get(position).getMessageSender()) {
            case "user":
                return 0;
            case "bot":
                return 1;
